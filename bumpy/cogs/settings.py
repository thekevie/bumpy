...
class menu(discord.ui.View):
    ...
    async def on_button_callback(self, button, interaction):
        ...
        em.add_field(name="Bump Channel", value=self.client.get_channel(db['channel_id']).mention, inline=True)
        em.add_field(name="Invite Channel", value=self.client.get_channel(db['invite_channel']).mention, inline=True)
    ...
    async def bump_channel_button_callback(self, button, interaction):
        ...
        i = await self.ctx.respond(embed="Mention the channel you want bumps to be sent in!", ephemeral=True)
    ...
    async def descriotion_button_callback(self, button, interaction):
        ...
        await i.edit(f"Server Description Was Added")
    ...
    async def update_button_callback(self, button, interaction):
      ...
      em.add_field(name="Invite Channel", value=self.client.get_channel(db['invite_channel']).mention, inline=True)
      em.add_field(name="Description", value=db['ad'], inline=False)
      await interaction.respons.edit_message(embed=em)
      self.ctx.send("Saved Changes")
    ...
    async def cancel_button_callback(self, button, interaction):   
      ...
      self.ctx.send("Cancelled Changes")
...
class settings(commands.Cog):
    ...
    @slash_command(description="A command to chnage the settings", guild_ids=[875451993462825000])
    ...
    @slash_command(description="A list of all commands")
    async def help(self, ctx: discord.ApplicationContext):
      em = discord.Embed(title='Help', description='`bump` `setup` `support` `invite` `vote`\n `report` `perks` `codes` `claim` `partners`\n `enable` `disable`', color=discord.Color.blue())
    ...
    @slash_command(aliases=['invite','support'], description="Some info about the bot")
